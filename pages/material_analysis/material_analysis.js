...
const registerEventListeners = () => {
  ...
  $("#productId").on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
...
const updateEntry = async () => {
  ...
  const response = await Request.send("/api/material_analysis", "PUT", { data: data });
...
const addToMaterialTable = () => {
  ...
  if (amount.trim() == "" || isNaN(amount) || !/^[\d]{1,7}\.[\d]{2}$/.test(amount)) {
...
const showMaterials = async (productId) => {
  const response = await Request.send(`/api/material_analysis?data[productId]=${productId}`, "GET");
