class employeeForm extends Form {
	...
	loadEntry(entry) {
		...
		if (
			$(`#${this.formId} #employeeStatusId option:selected`).text() == "Deleted"
		) {
...
async function loadModule(permissionStr) {
	...
	// load main form
	window.mainForm = new employeeForm(
		"mainForm",
		"Employee Details",
		permission,
		validationInfo,
...
const addEntry = async () => {
	...
	if (response.status) {
		...
		mainWindow.showOutputModal(
			"New Employee Added!",
			`<h5>Employee Number: ${response.data.number}</h5>`
		);
	}
};
...
const deleteEntry = async (id = mainForm.selectedEntry.id) => {
	...
	if (confirmation) {
		const response = await Request.send("/api/employees", "DELETE", {
			data: { id: id },
		});
		if (response.status) {
			reloadModule();
