class customerForm extends Form {
	...
	updateFormUI(customerTypeId) {
		if (customerTypeId == 1) {
			...
			$(`#${this.formId} label[for=customerName]`).text(
				"Contact Person Name: "
			);
...
async function loadModule(permissionStr) {
	...
	const dataBuilderFunction = (responseData) => {
		...
		return responseData.map((entry) => {
			return {
				...
				Delete: `${
					entry.customerStatus.name == "Deleted"
						? ""
	...
	// load main form
	window.mainForm = new customerForm(
		"mainForm",
		"Customer Details",
		permission,
		validationInfo,
		[
			{
				id: "customerStatusId",
				route: "/api/general?data[table]=customer_status",
				statusField: true,
			},
...
const showEditEntryModal = async (id, readOnly = false) => {
	...
	if (readOnly) {
		...
		$("#modalMainFormTitle").text("View Customer");
	} else {
...
const addEntry = async () => {
	...
	if (response.status) {
		...
		mainWindow.showOutputModal(
			"New Customer Added!",
			`<h5>Customer Number: ${response.data.number}</h5>`
		);
	}
};
...
const updateEntry = async () => {
	...
	const response = await Request.send("/api/customers", "PUT", {
		data: newEntryObj,
	});
...
const deleteEntry = async (id = mainForm.selectedEntry.id) => {
	...
	if (confirmation) {
		const response = await Request.send("/api/customers", "DELETE", {
			data: { id: id },
		});
		if (response.status) {
			reloadModule();
