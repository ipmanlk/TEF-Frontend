...
const showEditEntryModal = async (id, readOnly = false) => {
	...
	const response = await Request.send("/api/designations", "GET", {
		data: { id: id },
	});
	...
	if (readOnly) {
		...
		$("#modalMainFormTitle").text("View Designation");
	} else {
...
const addEntry = async () => {
	...
	// get response
	const response = await Request.send("/api/designations", "POST", {
		data: data,
	});
...
const updateEntry = async () => {
	...
	const response = await Request.send("/api/designations", "PUT", {
		data: newEntryObj,
	});
...
const deleteEntry = async (id = mainForm.selectedEntry.id) => {
	...
	if (confirmation) {
		const response = await Request.send("/api/designations", "DELETE", {
			data: { id: id },
		});
		if (response.status) {
			reloadModule();
