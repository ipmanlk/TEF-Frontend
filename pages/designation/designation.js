...
async function loadModule(permissionStr) {
    ...
    window.mainTable = new DataTable("mainTableHolder", "/api/designations", permission, dataBuilderFunction, "Designation List");
...
const showNewEntryModal = () => {
    mainForm.reset();
    $("#modalMainFormTitle").text("Add New Designation");
    $("#modalMainForm").modal("show");
}

const showEditEntryModal = async (id, readOnly = false) => {
    // get entry data from db and show in the form
    const response = await Request.send("/api/designations", "GET", { data: { id: id } });
    ...
    $("#modalMainFormTitle").text("Edit Designation");
    $("#modalMainForm").modal("show");
}
...
const addEntry = async () => {
    ...
    // get response
    const response = await Request.send("/api/designations", "POST", { data: data });
...
const updateEntry = async () => {
    ...
    const response = await Request.send("/api/designations", "PUT", { data: newEntryObj });
...
const deleteEntry = async (id = mainForm.selectedEntry.id) => {
    ...
    if (confirmation) {
        const response = await Request.send("/api/designations", "DELETE", { data: { id: id } });
        if (response.status) {
            reloadModule();
            $("#modalMainForm").modal("hide");
            mainWindow.showOutputToast("Success!", response.msg);
