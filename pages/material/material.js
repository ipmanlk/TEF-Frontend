    ...
    const dataBuilderFunction = (responseData) => {
        ...
        return responseData.map(entry => {
            return {
                ...
                "Delete": `${entry.materialStatus.name == "Deleted" ? "" : `<button class="btn btn-danger btn-sm" onclick="deleteEntry('${entry.id}')"><i class="glyphicon glyphicon-edit" aria-hidden="true"></i> Delete</button>`}`
            }
        });
    }

    window.mainTable = new DataTable("mainTableHolder", "/api/materials", permission, dataBuilderFunction, "Material List");

    // load main from
    window.mainForm = new Form("mainForm", "Material Details", permission, validationInfo,
        [
            ...
            { id: "materialStatusId", route: "/api/general?data[table]=material_status", statusField: true },
        ],
        {
            addEntry: addEntry,
...
const showEditEntryModal = async (id, readOnly = false) => {
    ...
    const response = await Request.send("/api/materials", "GET", { data: { id: id } });
...
const addEntry = async () => {
    ...
    if (response.status) {
        ...
        mainWindow.showOutputModal("New Material Added!", `<h4>Code: ${response.data.code}</h4>`);
    }
}
...
const updateEntry = async () => {
    ...
    const response = await Request.send("/api/materials", "PUT", { data: newEntryObj });
...
const deleteEntry = async (id) => {
    ...
    if (confirmation) {
        const response = await Request.send("/api/materials", "DELETE", { data: { id: id } });
        if (response.status) {
            reloadModule();
            $("#modalMainForm").modal("hide");
            mainWindow.showOutputToast("Success!", response.msg);
