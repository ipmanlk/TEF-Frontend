...
const loadFormDropdowns = async () => {
	...
	response = await Request.send(
		"/api/general?data[table]=quotation_request_status",
		"GET"
	);
	...
	quotationRequestStatuses.forEach((qs) => {
		$("#quotationRequestStatusId").append(
...
const addEntry = async () => {
	...
	const response = await Request.send("/api/quotation_requests", "POST", {
		data: data,
	});
	...
	if (response.status) {
		...
		mainWindow.showOutputModal(
			...
			`<h4>Request Number: ${response.data.qrnumber}</h4>`
		);
	}
};
...
const loadEntry = async (id) => {
	...
	const response = await Request.send("/api/quotation_requests", "GET", {
		data: { id: id },
	});
	...
	FormUtil.selectDropdownOptionByValue(
		...
		entry.quotationRequestStatus.id
	);
    ...
    <tr>
        <td></td>
        <td data-material-id="${qrm.material.id}">${qrm.material.name} (${
			qrm.material.code
		})</td>
        <td><input type="checkbox" value="" class="chkRequested" ${
					qrm.requested ? "checked" : ""
				}></td>
        <td><input type="checkbox" value="" class="chkAccepted" ${
					qrm.accepted ? "checked" : ""
				}></td>
        <td><input type="checkbox" value="" class="chkReceived" ${
					qrm.received ? "checked" : ""
				}></td>
        <td>
...
const updateEntry = async () => {
	...
	// send post reqeust to save data
	const response = await Request.send("/api/quotation_requests", "PUT", {
		data: data,
	});
...
const deleteEntry = async (id = tempData.selectedEntry.id) => {
	...
	const response = await Request.send(
		`/api/quotation_requests?data[id]=${id}`,
		"DELETE"
	);
...
const showSupplierMaterials = async (supplierId) => {
	const response = await Request.send(
		`/api/supplier_materials?data[supplierId]=${supplierId}`,
		"GET"
	);
...
const showNewEntryModal = () => {
	...
	// empty qrnumber
	$("#mainForm #qrnumber").val(
		"Request number will be displayed after adding."
	);
	// show modal
