...
const showSupplierPurchaseOrders = async (
	supplierId,
	...
	purchaseOrders.forEach((po) => {
		$("#purchaseOrderId")
			.first()
			.append(
				`<option data-tokens="${po.pocode}" value="${po.id}">${po.pocode}</option>`
			);
	});
...
const showPurchaseOrderMaterials = async (quotationId) => {
	...
	const response = await Request.send(
		`/api/purchase_orders?data[id]=${quotationId}`,
		"GET"
	);
...
const showPurchaseOrderMaterialInfo = (materialId) => {
	const material = tempData.selectedPurchaseOrder.purchaseOrderMaterials.find(
...
const addEntry = async () => {
	...
	if (response.status) {
		...
		mainWindow.showOutputModal(
			"GRN added!.",
			`<h4>GRN Code: ${response.data.grncode}</h4>`
		);
	}
};
...
const loadEntry = async (id) => {
	...
	$("#supplierId").val(
		entry.purchaseOrder.quotation.quotationRequest.supplier.id
	);
	$("#supplierId").selectpicker("render");

	// load supplier purchase orders
	await showSupplierPurchaseOrders(
		entry.purchaseOrder.quotation.quotationRequest.supplier.id,
		""
	);
