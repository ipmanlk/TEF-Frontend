...
const showEditEntryModal = async (id, readOnly = false) => {
	...
	const response = await Request.send("/api/employee_statuses", "GET", {
		data: { id: id },
	});
	...
	if (readOnly) {
		...
		$("#modalMainFormTitle").text("View Status");
	} else {
...
const addEntry = async () => {
	...
	// get response
	const response = await Request.send("/api/employee_statuses", "POST", {
		data: data,
	});
...
const updateEntry = async () => {
	...
	const response = await Request.send("/api/employee_statuses", "PUT", {
		data: newEntryObj,
	});
...
const deleteEntry = async (id = mainForm.selectedEntry.id) => {
	...
	if (confirmation) {
		const response = await Request.send("/api/employee_statuses", "DELETE", {
			data: { id: id },
		});
		if (response.status) {
			reloadModule();
