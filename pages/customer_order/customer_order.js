...
async function loadModule(permissionStr) {
	...
	// if customer order id is provided in url
...
const loadFormDropdowns = async () => {
	...
	response = await Request.send(
		"/api/general?data[table]=customer_order_status",
		"GET"
	);
	...
	customers.forEach((cus) => {
		...
		$("#customerId").append(
			`<option data-tokens="${cus.number} - ${name}" value="${cus.id}">${name} (${cus.number})</option>`
		);
	});

	productPackages.forEach((pp) => {
		$("#productPackageId").append(
			`<option data-tokens="${pp.code} - ${pp.name}" value="${pp.id}">${pp.name} (${pp.code})</option>`
		);
	});

	customerOrderStatuses.forEach((cos) => {
		$("#customerOrderStatusId").append(
	...
	return new Promise((resolve, reject) => {
		...
		$("#modalAddCustomerIframe").on("load", () => {
			document.getElementById(
				"modalAddCustomerIframe"
			).contentWindow.mainWindow = mainWindow;
			document
				.getElementById("modalAddCustomerIframe")
				.contentWindow.loadModule(mainWindow.tempData.privileges["CUSTOMER"]);
		});
...
const showProductPackageInfo = async (productPackageId) => {
	const response = await Request.send(
		`/api/product_packages?data[id]=${productPackageId}`,
		"GET"
	);
...
const addEntry = async () => {
	...
	const response = await Request.send("/api/customer_orders", "POST", {
		data: data,
	});
...
const updateEntry = async () => {
	...
	// send post reqeust to save data
	const response = await Request.send("/api/customer_orders", "PUT", {
		data: data,
	});
...
const deleteEntry = async (id = tempData.selectedEntry.id) => {
	...
	// send post reqeust to save data
	const response = await Request.send(
		`/api/customer_orders?data[id]=${id}`,
		"DELETE"
	);
...
const printEntry = async () => {
	...
	const response = await Request.send(
		`/api/customers?data[id]=${tempData.selectedEntry.customer.id}`,
		"GET"
	);
...
const getFormData = () => {
	...
	$("#productPackageTable tbody tr").each((i, tr) => {
		...
		const tdProductPackageSalePrice = $(tds[2])
			.children()
			.first()
			.data("product-package-sale-price");
		const tdProductPackageQty = $(tds[3])
			.children()
			.first()
			.data("product-package-qty");
...
const addToProductPackageTable = () => {
	...
	if (productPackageQty.trim() == "" || !/^[\d]+$/.test(productPackageQty)) {
		mainWindow.showOutputModal(
			"Sorry",
			"Please provice a valid product package quantity!."
		);
		return;
	}
...
const showEditEntryModal = (id, readOnly = false) => {
	loadEntry(id).then(() => {
		...
		if (readOnly) {
			...
			$("#mainForm *").removeClass("has-error has-success");
			$("#mainForm .form-control-feedback").remove();
			$("#modalMainFormTitle").text("View Customer Order");
		} else {
			FormUtil.disableReadOnly("mainForm");
