class DataTable {
  constructor(parentId, apiRoute, permission, dataBuilderFunction, tableTitle = "") {
  ...
  setRowListeners() {
    ...
    // hover highlight
    $(`#${this.parentId}-dt-tbody tr`).hover(function () {
  ...
  print() {
    ...
    win.document.write(`<html><head><link rel="stylesheet" href="${stylesheet}"></head><body><h2>${this.tableTitle}</h2><hr>${tableHTML}</body></html>`);
    setTimeout(() => {
      // element selectors to hide in the print view
      const classNames = [".dt-sm-label", `.dt-Delete-${parentId}-col`, `.dt-Edit-${parentId}-col`, `.dt-View-${parentId}-col`];

      // delete nodes with above selectors from print document
      classNames.forEach(className => {
        win.document.querySelectorAll(className).forEach(e => e.parentNode.removeChild(e));
      });
      win.print();
      win.document.close();
      win.close();
